...
for music_idx in range(1, 13):
    ...
    for i in range(1, 3):
        ...
        ax.axhline(y=0, color='r', linestyle='--')
        ax.set_ylim(-1.7, 1.7)
        ax.set_title(f'Principle Component {i} ({warp_func_pc_explained_ratio[i - 1]:.2%})')
    plt.tight_layout()
    plt.savefig(f'./output/plot/music{music_idx}/music{music_idx}_warp_func_pc.png', dpi=300)
    plt.close()
    # Then generate the Mean Variation plot (mean function +- PC function)
    warp_func_fpca_mean_pc_plot = FPCAPlot(
        basis_warp_func_fd.mean(),
        warp_func_fpca.components_,
        fig=plt.figure(figsize=(2 * 6, 1 * 4)),
        n_rows=1
    ).plot()
    i = 0
    for ax, new_title in zip(warp_func_fpca_mean_pc_plot.get_axes(), labels):
        ax.set_title(f'{labels[i]} ({warp_func_pc_explained_ratio[i]:.2%})')
        ax.set_ylim(-1.3, 2.45)
        i += 1
    plt.tight_layout()
    plt.savefig(f'./output/plot/music{music_idx}/music{music_idx}_warp_func_pc_var.png', dpi=300)
    plt.close()
